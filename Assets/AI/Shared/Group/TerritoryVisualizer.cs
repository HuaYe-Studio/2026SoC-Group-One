...
/// <summary>
/// [调试] 领地可视化：在 Scene 视图绘制所有已分配领地圆，用于目视确认领地分布与重叠情况。
...
public class TerritoryVisualizer : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            count++;
            Debug.Log($"[Territory] {t.OwnerKey} Center={t.Center} Radius={t.Radius} IsShared={t.IsShared}");
        }
        Debug.Log(count == 0
    ...
    private void OnDrawGizmos()
    {
        foreach (Territory t in TerritoryManager.All())
        {
            ...
            Gizmos.DrawWireSphere(center, t.Radius);
